...
int main()
   ...
   void writefile()
   {
	  int c;
	  FILE *fp;
	  fp = fopen("info.dat", "ab");						//opens file in append mode
	  fflush(stdin);									//empty the buffer
...
void search()
   {
	 ...
	 int n, i, m = 1;
	 FILE *fp;
	 fp = fopen("info.dat", "rb");						//open file in read binary mode
...
void update()
{
	char ch;
	int n, c;
	FILE *fp = fopen("info.dat", "rb+");				//opens file in read+ binary mode
